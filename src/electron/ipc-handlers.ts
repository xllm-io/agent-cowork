...
export function handleClientEvent(event: ClientEvent) {
  ...
  if (event.type === "session.start") {
    // Project mode: use the provided cwd. Conversation mode: cwd is undefined and
    // stays empty in the DB — the runner falls back to a temporary sandbox dir at
    // run time. This lets the frontend group sessions by whether they have a cwd.
