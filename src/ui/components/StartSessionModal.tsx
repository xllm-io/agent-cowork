...
const QUICK_TEMPLATES = [
  { label: "Code Review", prompt: "Please review the code in this project and identify potential issues, improvements, and best practices." },
  { label: "Debug Issue", prompt: "I need help debugging an issue. Let me describe the problem...", },
  ...
  return (
    <Dialog.Root open={true} onOpenChange={(open) => !open && onClose()}>
      <Dialog.Portal>
        ...
        <Dialog.Content className="fixed left-1/2 top-1/2 z-50 w-full max-w-lg -translate-x-1/2 -translate-y-1/2 rounded-2xl border border-ink-900/5 bg-surface p-6 shadow-elevated animate-fade-in-scale">
          ...
          <div className="mt-5 grid gap-4">
            ...
            {/* Quick templates */}
            <div>
              <div className="text-[11px] font-medium uppercase tracking-wide text-muted-light mb-2">Quick Templates</div>
              <div className="grid grid-cols-2 gap-2">
                {QUICK_TEMPLATES.map((tpl) => (
                  <button
                    ...
                    className={`group relative flex flex-col items-start gap-1.5 px-3.5 py-3 rounded-lg border text-left transition-all duration-200 ${
                      selectedTemplate === tpl.label
                        ? "bg-accent-subtle border-accent/40"
                        : "bg-surface border-ink-900/5 hover:border-accent/20 hover:bg-surface-tertiary"
                    }`}
                  >
                    <div className="flex items-center justify-between w-full">
                      <span className={`text-sm font-medium ${selectedTemplate === tpl.label ? "text-accent" : "text-ink-700 group-hover:text-ink-800"}`}>
                        {tpl.label}
                      </span>
                      <svg viewBox="0 0 24 24" className={`w-3.5 h-3.5 transition-all duration-200 ${selectedTemplate === tpl.label ? "text-accent translate-x-0 opacity-100" : "text-muted -translate-x-1 opacity-0 group-hover:translate-x-0 group-hover:opacity-100"}`} fill="none" stroke="currentColor" strokeWidth="2">
                        <path d="M5 12h14M12 5l7 7-7 7" />
                      </svg>
                    </div>
                    <p className="text-xs text-muted line-clamp-2">{tpl.prompt}</p>
                  </button>
                ))}
              </div>
            </div>
